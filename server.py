...
def get_Surahs(lang:str):
    ...
    if str(lang.strip())  not in ['ar','eng','fr','ru','de','es','tr','cn','fa','pt','sw']:
        return {"Error":"Invalid Language Code"}
    
    else:
            
        res = r.get(f'https://mp3quran.net/api/v3/suwar?language={str(lang.strip())}').json()
        surahs = []
...
def readers(lang:str):
    ...
    if str(lang.strip())  not in ['ar','eng','fr','ru','de','es','tr','cn','fa','pt','sw']:
        ...
        res = r.get(f"https://mp3quran.net/api/v3/reciters?language={str(lang.strip())}").json()
        readers = []
...
def reader_surahs(lang:str,reader_id:int):
    ...
    if str(lang.strip())  not in ['ar','eng','fr','ru','de','es','tr','cn','fa','pt','sw']:
        ...
        res = r.get(f"https://www.mp3quran.net/api/v3/reciters?language={str(lang.strip())}&reciter={str(reader_id)}").json()
...
def surah_link(lang:str,reader_id:int,surah_id:int):
    ...
    if str(lang.strip())  not in ['ar','eng','fr','ru','de','es','tr','cn','fa','pt','sw']:
        ...
        avs = r.get(f"https://www.mp3quran.net/api/v3/reciters?language={str(lang.strip())}&reciter={str(reader_id)}").json()
        av = avs['reciters'][0]["moshaf"][0]["surah_list"].split(',')
        if str(surah_id)  in av :
            res = r.get(f"https://www.mp3quran.net/api/v3/reciters?language={str(lang)}&reciter={str(reader_id)}").json()
